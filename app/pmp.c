...
static inline uint32_t read_pmpaddr(int idx)
{
    uint32_t val;
    switch (idx) {
    case 0: asm volatile("csrr %0, 0x3B0" : "=r"(val)); break;
    case 1: asm volatile("csrr %0, 0x3B1" : "=r"(val)); break;
    case 2: asm volatile("csrr %0, 0x3B2" : "=r"(val)); break;
    case 3: asm volatile("csrr %0, 0x3B3" : "=r"(val)); break;
    case 4: asm volatile("csrr %0, 0x3B4" : "=r"(val)); break;
    case 5: asm volatile("csrr %0, 0x3B5" : "=r"(val)); break;
    default: val = 0; break;
    }
    return val;
}
...
void task_c(void)
{
    ...
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        ...
        printf("Task C: CSR state: cfg0=0x%08x addr0=0x%08x addr1=0x%08x "
               "addr2=0x%08x\n",
